...
class Downloader:
    def __init__(self, url: str, live_manager: LiveManager, args=None) -> None:
        ...
        self.download_path = Path(args.custom_path) if args and args.custom_path else Path.cwd() / DOWNLOAD_FOLDER
        self.download_path.mkdir(parents=True, exist_ok=True)

    async def download_item(self, client: httpx.AsyncClient, current_task: int, file_info: dict) -> None:
        ...
        try:
            async with client.stream("GET", file_info["download_link"], headers=headers, timeout=30.0) as response:
                if check_response_status(response, filename):
                    task_id = self.live_manager.add_task(current_task=current_task)
                    await save_file_with_progress(response, final_path, task_id, self.live_manager)
        except httpx.TimeoutException:
            self.live_manager.update_log(event="Timeout", details=f"{filename} timed out")
        except Exception as e:
            self.live_manager.update_log(event="Error", details=f"{filename}: {e}")

    def parse_links(self, identifier: str, files_info: list, password: str | None = None, base_path: Path | None = None) -> None:
        ...
        with httpx.Client(timeout=10.0) as client:
            response = client.get(generate_content_url(identifier, password), headers=headers)
            data = response.json()

        if data["status"] != "ok":
            self.live_manager.update_log(event="Failed", details=f"Invalid response for {identifier}")
            return

        content = data["data"]
        if "password" in content and content.get("passwordStatus") != "passwordOk":
            self.live_manager.update_log(event="Password required", details="Invalid password")
            return
        ...
        if content["type"] == "folder":
            ...
            for child in content["children"].values():
                if child["type"] == "folder":
                    ...
                    files_info.append({"download_path": str(folder_path), "filename": child["name"], "download_link": child["link"]})
        else:
            files_info.append({"download_path": str(base_path), "filename": content["name"], "download_link": content["link"]})

    async def download_all(self, files_info: list) -> None:
        async with httpx.AsyncClient(limits=httpx.Limits(max_connections=MAX_WORKERS)) as client:
            tasks = [self.download_item(client, i, file_info) for i, file_info in enumerate(files_info)]
    ...
    def initialize_download(self) -> None:
        ...
        files_info = []
        hashed_password = hashlib.sha256(self.password.encode()).hexdigest() if self.password else None
        self.parse_links(content_id, files_info, hashed_password)

        if not files_info:
            return

        self.live_manager.add_overall_task(description=content_id, num_tasks=len(files_info))
