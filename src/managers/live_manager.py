...
class LiveManager:
    def __init__(self, progress_manager: ProgressManager, logger_table: LoggerTable, refresh_per_second: int = 10) -> None:
        self.progress_manager = progress_manager
        self.progress_table = self.progress_manager.create_progress_table()
        self.logger_table = logger_table
        self.live = Live(self._render_live_view(), refresh_per_second=refresh_per_second)
    ...
    def stop(self) -> None:
        ...
        self.update_log(event="Completed", details=f"Time: {hrs:02}:{mins:02}:{secs:02}")
        self.live.stop()
    ...
    def _render_live_view(self) -> Group:
        ...
        return Group(self.progress_table, self.logger_table.render_log_panel(panel_width=2 * panel_width), footer)
